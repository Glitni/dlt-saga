...
| `initial_value` | — | Cursor value used to seed the first-run scan when the state log has no entry for this pipeline yet — equivalent to dlt's `dlt.sources.incremental(initial_value=...)`. Format matches `filename_date_format`. Only honored when `incremental: true` and date fields are set. Ignored on subsequent runs (the real cursor from the state log wins). Use to backfill from a date earlier than today on the first run; dates that predate the bucket's earliest file walk empty partitions harmlessly. |
