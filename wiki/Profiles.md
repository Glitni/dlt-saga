...
| `partition_expiration_days` | No | — | Default partition expiration (in days) for partitioned tables created by pipelines targeting this profile. Maps to BigQuery's `time_partitioning.expiration_ms`. Honored on `CREATE TABLE`; can be overridden per-pipeline. Only applies to native tables — Iceberg ignores it. |
