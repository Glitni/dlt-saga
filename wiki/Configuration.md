...
| `partition_expiration_days` | int | inherited from profile | BigQuery only. Sets `time_partitioning.expiration_ms` on the created table. Honored on first `CREATE TABLE` for both dlt-managed pipelines and `native_load`. Pipeline-level value overrides the profile default. Has no effect on Iceberg tables. |
