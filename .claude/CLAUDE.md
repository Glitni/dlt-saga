...
- `_dlt_ingested_at` column injected by `BasePipeline._inject_ingested_at()` for ALL `append` and `replace` pipelines (handles both dict rows and PyArrow tables); excluded for `merge`/`scd2` (dlt manages its own columns, and an injected timestamp would trigger false change detection). This makes `replace+historize` work — each `replace` run overwrites the staging table with one snapshot stamped at run time. Auto-clustering by `_dlt_ingested_at` is append-only (a replace table holds a single ~constant timestamp). Caveat: a `--full-refresh` historize over a `replace` source rebuilds from only the latest snapshot — history accrues going forward, run by run.
